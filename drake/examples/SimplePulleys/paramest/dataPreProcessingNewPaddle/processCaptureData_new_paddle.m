function [] = processCaptureData_version2(filename,expStartTime,expEndTime,maxHeightAfterScaling,...
...
filenameWithExtension = [filename,'.pts'];
[Frame,t,x,z,xd,zd,psi1] = importfile(filenameWithExtension);
...
%Time values
timeStepsOrig = t(1:numberOfSamples)';
%height values
posDiscOrig = [z(1:numberOfSamples),x(1:numberOfSamples),zeros(numberOfSamples,1)]; % x points up on the plane
...
posDisc(:,3) = posDisc(:,3) - touchPointAfterScaling + discRadius;% contact in the urdf model happens when z = discradius
posDisc(:,1) = -(posDisc(:,1) - horizOffset); 
