...
x0 = Point(getStateFrame(p));
x0.m = 1;
x0.load_x = -0.00129928588867188-dSpatial/2;
x0.load_z = 0.4672;
% x0.load_zdot = -4;
x0 = double(x0);
x0(2:end) = resolveConstraints(p.no_contact,x0(2:end));
...
tic
[ytraj,xtraj] = simulate(sys,[0 1.0],x0);
toc
tt=getBreaks(ytraj);
yAll = ytraj.eval(tt);
...
% B = deltapsi([3:4,7:8]);
A = [deltax([3:4,7]),deltaz([3:4,7]),deltaxdot([3:4,7])];
B = deltapsi([3:4,7]);
