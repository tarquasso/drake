classdef SoftPaddlePositionControllerReal < DrakeSystem
    properties
        kp
        kd
        plant
    ...
    methods
        function obj = SoftPaddlePositionControllerReal(plant)
...
%             Hbtilde = 1/2*xp^2 + 9.81*sind(36)*(z-zmax);
            Hbtilde = 1/2*xp^2 + 9.81*(z-zmax);
            xFixed = -0.00129928588867188;
            
            psid = obj.psiDes;
%             psid = obj.gamma1*(x-xFixed) + obj.gamma2*(z-zmax) + obj.gamma3*xp + obj.gamma4*Hbtilde;
