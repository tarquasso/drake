classdef SoftPaddleControlReal < DrakeSystem
  properties
    kp
    kd
    plant
  ...
  methods
    function obj = SoftPaddleControlReal(plant)
...
%       K = -0.1*[0, -0.16525, -0.19829, 0.0015943];
...
%       assignin('base','psid',temp);
    ...
    function u = output(obj,t,~,x)
      ...
      epsilon = 0.5;
      xFixed = -0.00129928588867188;
%       xFixed = -0.0025;
      zTouchDes = 0;
      
      psi = q(1); psip = qp(1);
      R = [cos(psi), sin(psi); -sin(psi), cos(psi)];
      load_in_paddle = -[0.145; 0] + R'*([q(3); q(4)]-[-0.145;0]);
      loadv_in_paddle = R'*[qp(3); qp(4)];
      
      tf = max(roots([-1/2*9.81*cos(psi), loadv_in_paddle(2), load_in_paddle(2)-0.04423]));
      zTouch = q(4) + qp(4)*tf - 1/2*9.81*tf^2;
      zpTouch = qp(4) - 9.81*tf;
      xTouch = q(3) + qp(3)*tf;
      xpTouch = qp(3);
      
      Kgamma =[     -0.37922      -1.1764      0.37334
      0.16887      0.38959       -0.146
      0.16887      0.38959       -0.146
      0.16887      0.38959       -0.146];
      
	  x = q(3); z = q(4); xp = qp(3); zp = qp(4);
      zmax = 0.35;
      
      if zp > 0 && m == 1
        xpeak = x + xp*zp/9.81;
        zpeak = z + (zp^2)/2/9.81;
        assignin('base', 'xpeak', xpeak);
        assignin('base', 'zpeak', zpeak);
      end
      xpeak = evalin('base', 'xpeak');
      zpeak = evalin('base', 'zpeak');
            
      vec = [x-xFixed; z-zmax; xp];
      Gamma = Kgamma*vec;
      
      
%       gamma1 = Kgamma(1,1)*(x-xFixed) + Kgamma(1,2)*(z-zmax) + Kgamma(1,3)*xp;
%       gamma2 = Kgamma(2,1)*(x-xFixed) + Kgamma(2,2)*(z-zmax) + Kgamma(2,3)*xp;
%       gamma3 = Kgamma(3,1)*(x-xFixed) + Kgamma(3,2)*(z-zmax) + Kgamma(3,3)*xp;
%       gamma4 = Kgamma(4,1)*(x-xFixed) + Kgamma(4,2)*(z-zmax) + Kgamma(4,3)*xp;
%       gamma4 = 10*gamma4;
      
      Hbtilde = 1/2*xp^2 + 9.81*(z-zmax);
...
%           psid = obj.K(1)*(xTouch-xFixed) + obj.K(2)*(zTouch-zTouchDes) + obj.K(3)*xpTouch + obj.K(4)*(zpTouch);    % (Linearized) Poincare map controller
          psid = obj.K(1)*(xTouch-xFixed) + obj.K(2)*(zTouch-zTouchDes) + obj.K(3)*xpTouch;    % (Linearized) Poincare map controller
          psid = obj.K(1)*(xpeak-xFixed) + obj.K(2)*(zpeak-zmax) + obj.K(3)*xp;
%       else
%           psid = gamma1*(x-xFixed) + gamma2*(z-zmax) + gamma3*xp + gamma4*Hbtilde*0;
%           psid = -0.01*(x-xFixed) + 0.038*(z-zmax) - 0.01*xp;
%           psid = Gamma'*vec;
%       end
...
%       end
      ...
      R = [cos(psi), sin(psi); -sin(psi), cos(psi)];
      Rp = psip*[-sin(psi), cos(psi); -cos(psi), -sin(psi)];
      
      load_in_paddle = [ox; 0] + R'*([x; z]-[ox;0]);
      loadv_in_paddle = Rp'*[x;z] + R'*[xp; zp];
      ...
      k1 = 0.25; k2 = 0.5;
      psiImpact = -k1*(xTouch-xFixed) - k2*xpTouch;
      psiImpact = 0;
      
      tl = max(roots([-1/2*9.81*cos(psiImpact), xp*sin(psiImpact) + zp*cos(psiImpact), ((x-ox)*sin(psiImpact) + z*cos(psiImpact))-obj.plant.radius]));
      xl = x + xp*tl;
      kappa = 0.4 + 0.001*Htilde;
      
      if abs(z - obj.plant.radius) < 0.15
          psid = real(1/2*asin((2*kappa*(z-obj.plant.radius))/(xl-ox)));
          if psid > 0.26
              psid = 0.26;
          elseif psid < -0.26
              psid = -0.26;
      ...
      K = [-2.117, -0.0034, -0.512];
      psid = K(1)*(xpeak - xFixed) + K(2)*(zpeak-zmax) + K(3)*xp;
