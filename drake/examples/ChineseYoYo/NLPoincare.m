...
psiGuess = [-0.011;0;-0.008;-0.015;-0.023];
fun = @(psi) costFunCumulative(psi,x0,plantSim,c);
options = optimoptions(@fmincon);
options = optimoptions(options, 'Display', 'iter');
A = []; b = []; Aeq = []; beq = []; nonlcon = [];
% lb = [-0.001; -0.08; -0.06; -0.04; -0.02]; ub = [0.2; 0.08; 0.06; 0.04; 0.02];
% lb = -0.05*ones(5,1); ub = 0.05*ones(5,1);
lb = [-0.11; -0.1; -0.08; -0.08; -0.08]; ub = [0.05; 0.1; 0.06; 0.06; 0.06];
[psid, fval, exitflag, output, grad, hessian] = fmincon(fun, psiGuess, A, b, Aeq, beq, lb, ub, nonlcon, options)
end