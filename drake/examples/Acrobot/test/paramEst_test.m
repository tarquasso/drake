...
%   paramNames = getCoordinateNames(r.getParamFrame);
  ...
  parameterEstimationOptions = struct();
  ...
  sampSize = k*500;
  Ts = 1./sampSize; tsamples = T0:Ts:Tf;
  ...
  %         end
          ...
          fprintf('k: %i \tn: %i \tModel: %10s \tTime: %f \tSim Error: %f\n',k,j,models{i},simtime(i,j),simerror(i,j));
      end
  end
  ...
  disp(meanSimerror);
  disp(meanSimtime);
  fname=sprintf('data/data%d',k);
  fprintf('--------------Saving...');
  parsave(fname,'simerror','simtime','simflag','p_true','p_init','p_est');
  fprintf('--------------Saved');
end
end

function parsave(fname, x1,x2,x3,x4,x5,x6)
save(fname, 'x1','x2','x3','x4','x5','x6');
end
