...
if isDynamic || isEnergetic
  ...
  elseif isEnergetic
    ...
    if ~isa(obj,'AcrobotPlant')
      error('Only Acrobot energy dissipation is implemented so far');
    end
...
%   ff = extractTrigPolySystem(pobj,options);
%   f = [qd;inv(qdd_den)*qdd_num];
...
% required for computing dfdx and dfdp
  xvar = [q;qd];
  qdd_n_dx = diff(qdd_n,xvar);
  qdd_d_dx = diff(qdd_d,xvar);
  qdd_n_dp = diff(qdd_n,p);
  qdd_d_dp = diff(qdd_d,p);
  
  
% Faster substitution method adopted from Michael Posa
  vars = [p,q,qd,u,t];
  [qdd_n_pows,qdd_n_coeffs] = decomp_ordered(qdd_n,vars);
  qdd_n_decomp = struct('mss',qdd_n,'pows',qdd_n_pows,'coeffs',qdd_n_coeffs,'vars',vars);
  [qdd_d_pows,qdd_d_coeffs] = decomp_ordered(qdd_d,vars);
  qdd_d_decomp = struct('mss',qdd_d,'pows',qdd_d_pows,'coeffs',qdd_d_coeffs,'vars',vars);
  
  [qdd_n_dx_pows,qdd_n_dx_coeffs] = decomp_ordered(qdd_n_dx,vars);
  qdd_n_dx_decomp = struct('mss',qdd_n_dx,'pows',qdd_n_dx_pows,'coeffs',qdd_n_dx_coeffs,'vars',vars);
  [qdd_d_dx_pows,qdd_d_dx_coeffs] = decomp_ordered(qdd_d_dx,vars);
  qdd_d_dx_decomp = struct('mss',qdd_d_dx,'pows',qdd_d_dx_pows,'coeffs',qdd_d_dx_coeffs,'vars',vars);
  
  [qdd_n_dp_pows,qdd_n_dp_coeffs] = decomp_ordered(qdd_n_dp,vars);
  qdd_n_dp_decomp = struct('mss',qdd_n_dp,'pows',qdd_n_dp_pows,'coeffs',qdd_n_dp_coeffs,'vars',vars);
  [qdd_d_dp_pows,qdd_d_dp_coeffs] = decomp_ordered(qdd_d_dp,vars);
  qdd_d_dp_decomp = struct('mss',qdd_d_dp,'pows',qdd_d_dp_pows,'coeffs',qdd_d_dp_coeffs,'vars',vars);
  
  
  dfdx = @(pval,qval,qdval,uval,tval) tval*[diff(qd,xvar);dHinvAdxFun(pval,qval,qdval,uval,tval,qdd_n_decomp,qdd_d_decomp,qdd_n_dx_decomp,qdd_d_dx_decomp,xvar)];
  dfdx(p,q_data(:,1),qd_data(:,1),u_data(:,1),t_data(:,1))
  dfdp = @(pval,qval,qdval,uval,tval) tval*[diff(qd,p);dHinvAdxFun(pval,qval,qdval,uval,tval,qdd_n_decomp,qdd_d_decomp,qdd_n_dp_decomp,qdd_d_dp_decomp,p)];
...
function dHinvAdx = dHinvAdxFun(p_data,q_data,qd_data,u_data,t_data,A,H,dAdx,dHdx,x)
%
% Uses the quotient/chain rule to calculate the derivative of Hinv*A w.r.t x
% d(inv(H)*A)/dx = -inv(H)*dH/dx*inv(H)+inv(H)*dA/dx
%
  vars = A.vars;
  in = [p_data;q_data;qd_data;u_data;t_data];
  
  Hval = msubs(getmsspoly(H),vars,in);
  Hvalinv = pinv(Hval);
  Aval = msubs(getmsspoly(A),vars,in);
  dAval = msubs(getmsspoly(dAdx),vars,in);
  
  
  dHinvAdx = zeros(size(q_data),length(x));
  for i=1:length(x)
    dHi = diff(qdd_den_val,x(i));
    dHival = msubs(getmsspoly(dHi),vars,in);
    dHinvAdx(:,i) = -Hvalinv*(dHival*Hvalinv*Aval+dAval);
  end
end
